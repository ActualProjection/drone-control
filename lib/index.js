...
function AROilFinder(client, ctrl, options) {
    ...
    setTimeout(this.start.bind(this, this.move_pattern.bind(this)), 1000);
    this._moving_direction = 1
    this._found_oil = false

    this.register_listeners()
}

AROilFinder.prototype.move_pattern = function(cb){
    // just move forward for now
    console.log('moving forward')
    this.ctrl.forward(1)
    // Don't continue if we've found 'oil'
    if (!this._found_oil){
        setTimeout(this.move_pattern.bind(this), this._options.refresh_rate)
    }
}
...
AROilFinder.prototype.found_indicator = function(){
    // blink for 4 seconds
